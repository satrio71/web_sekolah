...
class Foto extends CI_Controller {
	...
	{	
		...
		$data['album'] = $this->db->select('*')->from('tb_album')->where('is_active',1)->order_by('id_album','desc')->get();
  		$this->template->admin('admin/dashboard','admin/foto/foto', $data);
    }
        ...
        foreach($list as $r)
        {
            if($r->foto != '' AND file_exists("assets/img/foto/$r->foto"))
            {
				$img = '<a href="'.base_url("assets/img/foto/$r->foto").'" target="_blank">
						<img src="'.base_url("assets/img/foto/$r->foto").'" class="img img-fluid" style="object-fit:contain; width:100%; max-height:150px;"></a>'; 
            }else
            {
				$img = '';
			}
