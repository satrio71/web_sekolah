...
class Us extends CI_Controller {
	...
	{	
		$data['title'] = 'Rekap US / UM';
  		$this->template->admin('admin/dashboard','admin/rekap_us/rekap_us', $data);
	}
	...
	{	
		...
		foreach($list as $r)
		{
			...
			$action = '<div class="text-center">
						<a href="'.base_url("backend/edit-rekap-us/$r->id_us").'" class="btn btn-dark btn-xs btn-flat" title="EDIT DATA">EDIT</a>
	...
	{ 
		...
		$this->template->admin('admin/dashboard','admin/rekap_us/form_tambah_us',$data);
	}
	...
	{ 
		...
		$data['tahun'] = $this->db->select('*')->from('tb_tahun')->order_by('tahun','desc')->get();
		$data['mapel'] = $this->db->select('*')->from('tb_kurikulum')->where('is_active',1)->order_by('mapel','asc')->get();
		$this->template->admin('admin/dashboard','admin/rekap_us/form_edit_us',$data);
	}
